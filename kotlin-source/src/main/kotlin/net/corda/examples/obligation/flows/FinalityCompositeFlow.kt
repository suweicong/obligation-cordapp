 ...
 */
@InitiatingFlow
class FinalityCompositeFlow(val transaction: SignedTransaction,
...
//        progressTracker.currentStep = BROADCASTING
    ...
    private fun getPartiesToSend(ltx: LedgerTransaction): Set<Party> {
        ...
        val result = try {
            groupAbstractPartyByWellKnownParty(serviceHub, participants).keys
        } catch (ex: Exception) {
            // maybe is compositeKey
            val leavesKeys = participants
                    .map { it.owningKey as CompositeKey }
                    .flatMap { it.leafKeys }
                    .toSet()
            val leavesParties = leavesKeys.map {
                (serviceHub.identityService.partyFromKey(it) ?: throw IllegalArgumentException("Could not find Party for $it"))
            }

            leavesParties
        }
        println("result $result")
        println("extraRecipients $extraRecipients")
        val toSend = result.toSet() + extraRecipients
...
@InitiatedBy(FinalityCompositeFlow::class)
class FinalityCompositeHandler(private val sender: FlowSession) : FlowLogic<Unit>() {
    @Suspendable
    override fun call() {
        subFlow(ReceiveTransactionFlow(sender, true, StatesToRecord.ALL_VISIBLE))
    }
}
