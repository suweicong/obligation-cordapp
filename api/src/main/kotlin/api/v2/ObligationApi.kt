...
import flows.v2.IssueObligation
import flows.v2.SettleObligation
import flows.v2.TransferObligation
...
class ObligationApi(val rpcOps: CordaRPCOps) {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    fun owedPerCurrency() = rpcOps.vaultQuery(ObligationV2::class.java).states
    ...
    @Path("obligations")
    @Produces(MediaType.APPLICATION_JSON)
    fun obligations() = rpcOps.vaultQuery(ObligationV2::class.java).states
