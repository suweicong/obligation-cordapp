...
class ObligationContractV2 : Contract {

    companion object {
        @JvmStatic
        val OBLIGATION_CONTRACT_ID = "contract.states.v2.ObligationContractV2"
    }
    ...
    private fun keysFromParticipants(obligation: ObligationV2): Set<PublicKey> {
    ...
    private fun verifyIssue(tx: LedgerTransaction, signers: Set<PublicKey>) = requireThat {
        ...
        val obligation = tx.outputsOfType<ObligationV2>().single()
    ...
    private fun verifyTransfer(tx: LedgerTransaction, signers: Set<PublicKey>) = requireThat {
        ...
        val input = tx.inputsOfType<ObligationV2>().single()
        val output = tx.outputsOfType<ObligationV2>().single()
    ...
    private fun verifySettle(tx: LedgerTransaction, signers: Set<PublicKey>) = requireThat {
        ...
        val obligationInputs = tx.inputsOfType<ObligationV2>()
        ...
        val obligationOutputs = tx.outputsOfType<ObligationV2>()
